import time
import torch.optim
from lab4.dataset import MNISTMetricDataset
...
if __name__ == '__main__':
    ...
    if os.path.exists("model.pt"):
        print("Model file exits. Loading model.")
    ...
    print("Evaluating on image space")
    emb_size = 784
    model2 = IdentityModel().to(device)
    ...
    representations = compute_representations(model2, test_loader, num_classes, emb_size, device)

    acc1 = evaluate(model2, representations, test_loader, device)
    print(f"Image space accuracy: {round(acc1 * 100, 2)}%")

    print("Evaluating with removed class")

    ds_train = MNISTMetricDataset(mnist_download_root, split='train', remove_class=0)
    ds_test = MNISTMetricDataset(mnist_download_root, split='test', remove_class=0)
    ds_traineval = MNISTMetricDataset(mnist_download_root, split='traineval', remove_class=0)
    ...
    if os.path.exists("model-removed-class.pt"):
        print("Model file exits. Loading model.")
        model = SimpleMetricEmbedding(1, emb_size).to(device)
        model.load_state_dict(torch.load("model.pt"))
